...
struct stream_item
{
  string stream_name;
  string input_url;
  ServerMediaSession *stream_session;
};

vector<stream_item> currentListOfStreams;
...
void httpServerInit(int port)
{
  // Start HTTP-SERVER

  control_svr.Get("/hello", [](const Request &req, Response &res)
                  { res.set_content("Hello World!", "text/plain"); });
  control_svr.Post("/add_stream", [](const Request &req, Response &res)
                   {
                     bool isExist = false;
                     ...
                     try
                     {
                       // cout<<"body = "<<body["input_url"]<<endl;
                       if (!body.contains("input_url"))
                       {
                         result["message"] = "URL input not found";
                         res.set_content(result.dump(), "appliation/json");
                       }
                       else
                       {
                         for (int i = 0; i < currentListOfStreams.size(); i++)
                         {
                           if (body["input_url"].get<std::string>() == currentListOfStreams[i].input_url)
                           {
                             result["streamID"] = currentListOfStreams[i].stream_name;
                             isExist = true; break;
                           }
                         }
                         if(isExist)
                         {
                             result["message"] = "Input Already Exist";
                             res.set_content(result.dump(), "appliation/json");
                         }
                         else
                        {
                          // auto streamID = "proxyStream-" + std::to_string(currentListOfStreams.size() + 1);
                              string streamID = get_uuid();
                         // cout<<"streamID = "<<streamID<<endl;
                         ServerMediaSession *sms = ProxyServerMediaSession::createNew(*env, rtspServer,
                                                                                      body["input_url"].get<std::string>().data(), streamID.data(),
                                                                                      username, password, tunnelOverHTTPPortNum, verbosityLevel);
                         rtspServer->addServerMediaSession(sms);
                         rtspServer->rtspURL(sms);
                         stream_item item = {streamID, body["input_url"].get<std::string>(), sms};
                         currentListOfStreams.push_back(item);

                         result["message"] = "Stream added.";
  ...
  control_svr.Post("/remove_stream", [](const Request &req, Response &res)
                   {
                     json body = json::parse(req.body);
                     json result;
                     try
                     {
                       ...
                       {
                         result["message"] = "Please include streamID in the message.";
                         res.set_content(result.dump(), "appliation/json");
                       }
                       else
                       {
                         for (int i = 0; i < currentListOfStreams.size(); i++)
                         {
                           if (body["stream_id"].get<std::string>() == currentListOfStreams[i].stream_name)
                           {
                             rtspServer->removeServerMediaSession(currentListOfStreams[i].stream_session);
                             currentListOfStreams.erase(currentListOfStreams.begin() + i);
                           }
                         }
                         result["message"] = "Stream removed.";
                         res.set_content(result.dump(), "appliation/json");
                       }
                     }
                     catch (...)
  ...
  control_svr.Get("/get_streams", [](const Request &req, Response &res)
                  {
    json result;
     for (int i = 0; i < currentListOfStreams.size(); i++) {
        // cout << "Stream " << currentListOfStreams[i].stream_name << " found at index " << i << std::endl;
        result[currentListOfStreams[i].stream_name] = currentListOfStreams[i].input_url;
    }
    cout<<"result = "<< result <<endl;
    res.set_content(result.dump(), "appliation/json"); });
  control_svr.Get("/check_stream_status", [](const Request &req, Response &res)
 {
    std::string bash = "`ffprobe -v quiet -print_format json -show_streams -timeout 1000000 -loglevel quiet rtsp://yq1:abcd\\$1234@14.241.248.157:1554/unicast/c43/s0/live`";
    std::string in;
    ...
    // std::cin >> in;
    res.set_content(result.data(), "text/plain"); });
  control_svr.listen("0.0.0.0", port);
}
...
int main(int argc, char **argv)
{
  ...
  std::thread t1(httpServerInit, 1234);
  ...
  *env << "LIVE555 Proxy Server\n"
  ...
  while (argc > 1)
  {
    ...
    { // specify a username and password (to be used if the 'back end' (i.e., proxied) stream requires authentication)
      if (argc < 4)
        usage(); // there's no argv[3] (for the "password")
      username = argv[2];
      password = argv[3];
      argv += 2;
      argc -= 2;
  ...
  if (argc < 2 && !proxyREGISTERRequests)
    usage(); // there must be at least one URL at the end
  // Make sure that the remaining arguments appear to be "rtsp://" (or "rtsps://") URLs:
  int i;
  for (i = 1; i < argc; ++i)
  {
    if (strncmp(argv[i], "rtsp://", 7) != 0 && strncmp(argv[i], "rtsps://", 8) != 0)
      usage();
  }
  ...
  for (i = 1; i < argc; ++i)
  {
    ...
    cout << "proxiedStreamURL = " << proxiedStreamURL << endl;
    cout << "streamName = " << streamName << endl;

    char *proxyStreamURL = rtspServer->rtspURL(sms);
    stream_item item = {streamName, proxiedStreamURL, sms};
    currentListOfStreams.push_back(item);
