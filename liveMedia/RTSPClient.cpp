...
unsigned RTSPClient::sendRequest(RequestRecord* request) {
  ...
  do {
    ...
    // std::string stringToRemoveFromURL = std::string(auth.username()) + ":" + std::string(auth.password()) + "@";
    // std::string urlCopy = std::string(cmdURL);
    // std::string::size_type i = urlCopy.find(stringToRemoveFromURL);
    // if (i != std::string::npos)
    // urlCopy.erase(i, stringToRemoveFromURL.length());
    // char const* urlToResend = urlCopy.c_str();
    // envir() << "Command URL" << "\n";
    // envir() << urlToResend << "\n";
...
// so that other projects that use the code could benefit (not just your own project).
	...
	*responseEnd = saved;
    ...
    envir() << "We are here with the authen ";
